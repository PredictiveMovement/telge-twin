...
// Create a new experiment based on an existing one with updated settings
// This creates a new version that shares the same truck-plans (via vroomTruckPlanIds array).
// Plans are only copied when route order is modified (handled by route-order endpoint).
app.post('/api/experiments/:experimentId/fork', async (req, res) => {
  try {
    const { experimentId } = req.params

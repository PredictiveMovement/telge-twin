...
const osrm = {
  ...
  /**
   * Route through multiple waypoints and return total duration/distance plus leg summaries.
   * Coordinates are [lon, lat] pairs. No geometry decoding is performed.
   */
  async routeMultiWaypoint(
    ...
    return cachedFetch('routeMulti', { coordinates }, async () => {
      ...
      let totalDuration = 0
      let totalDistance = 0
      const totalLegs: Array<{ duration: number; distance: number }> = []
      ...
      return { duration: totalDuration, distance: totalDistance, legs: totalLegs }
    })
  },

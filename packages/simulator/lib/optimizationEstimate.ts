...
const DEFAULT_DEPOT_COORDINATE = { lat: 59.135449, lng: 17.571239 }
...
export async function estimateTruckRuntime(
  ...
  let currentTimeMs = truck.virtualTime.getWorkdayBounds().startMs
  ...
  for (const tour of pickupTours) {
    ...
    for (let index = 0; index < tour.length; index += 1) {
      ...
      ;({ currentTimeMs, breakIndex } = consumeDueBreaks(
        currentTimeMs,
        breaks,
        breakIndex
      ))
      ...
      currentTimeMs += CLUSTERING_CONFIG.SERVICE_TIME_PER_STOP_SECONDS * 1000
  ...
  return {
    durationSeconds: Math.ceil(
      (currentTimeMs - truck.virtualTime.getWorkdayBounds().startMs) / 1000
    ),
    distanceMeters,
...
export async function estimateOptimizationFeasibility(
  ...
  const estimates = await Promise.all(
    (request.fleetConfiguration || []).flatMap(async (fleet) => {
      ...
      return Promise.all(
        (fleet.vehicles || []).map(async (vehicle) => {
          ...
          const unreachableStopCount = bookings.filter(
            (booking) => booking.status === 'Unreachable'
          ).length
          ...
          return {
            ...
            unreachableStopCount:
              Math.max(unreachableStopCount, runtime.unreachableStopCount) || 0,
          }
        })
      )
    })

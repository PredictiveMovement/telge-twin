...
export function computeFeasibility(
  ...
  const workdayMinutes =
    endMinutes <= startMinutes
      ...
      : endMinutes - startMinutes
  const breakMinutes = [...breaks, ...extraBreaks].reduce((sum, breakItem) => {
    if (!breakItem?.enabled) return sum
    return sum + (Number.isFinite(breakItem.duration) ? breakItem.duration : 0)
  }, 0)
  const availableMinutes = Math.max(0, workdayMinutes - breakMinutes)

  ...
  return (
    <Card className={cn('p-4', getStatusStyles(), className)}>
      <CardContent className="p-0 space-y-3">
        {showOrder && (
          <div className="flex items-center gap-2 mb-2">
            ...
            {booking.stepIndex !== undefined && (
              ...
              >
                {orderLabel || 'VROOM'} #{booking.stepIndex + 1}
              </Badge>
            )}
          </div>
        )}
        ...
        {booking.standardBookingId && (
          <div className="pt-2 border-t border-gray-100 text-xs text-gray-400">
            <div>Standard ID: {booking.standardBookingId}</div>
            {booking.fullId && booking.fullId !== booking.id && (
              <div>Full ID: {booking.fullId}</div>
            )}
          </div>
        )}
      </CardContent>

...
const FeasibilityIndicator: React.FC<FeasibilityIndicatorProps> = ({
  ...
  const hasUnreachableStops = vehicles.some(
    (vehicle) => vehicle.unreachableStopCount > 0
  )
  const totalUnreachableStops = vehicles.reduce(
    (sum, vehicle) => sum + vehicle.unreachableStopCount,
    0
  )
  ...
  const summaryText = allFit
    ...
    : hasUnreachableStops
      ? `${totalUnreachableStops} stopp ryms inte inom arbetstiden`
      : hasOverflows
...
const VehicleRow: React.FC<{ vehicle: VehicleFeasibility }> = ({
  ...
  return (
    ...
    >
      ...
      {vehicle.unreachableStopCount > 0 && (
        <span className="text-xs font-medium text-telge-morkrod w-[88px] text-right">
          {vehicle.unreachableStopCount} ej körbara
        </span>
      )}
    </div>
  )
